...
{
	internal static class OrderSubmitEmailConstants
	{
		...
		{
			Dictionary<VerifiedUserType, EmailDisplayText> dictionary = new Dictionary<VerifiedUserType, EmailDisplayText>()
			{
				{
					VerifiedUserType.buyer, new EmailDisplayText()
					{
						...
						DynamicText2 = @"We are getting your order ready to be shipped. You will be notified when it has been sent. Your order contains the following items."
					}
				},
				{
		...
		public static EmailDisplayText GetQuoteRequestConfirmationText()
		{
			return new EmailDisplayText()
			{
				...
				DynamicText2 = @"You will receive a notification once your quote request has been quoted."
			};
		}
