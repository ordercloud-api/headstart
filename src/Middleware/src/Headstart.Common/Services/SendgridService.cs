	...
	public class SendgridService : ISendgridService
	{
		...
		public async Task SendQuoteRequestConfirmationEmail(HsOrder order, HsLineItem lineItem, string buyerEmail)
		{
			try
			{
				...
				{
					Data = SendgridMappers.GetQuoteOrderTemplateData(order, new List<HsLineItem>
					{
						lineItem
					}),
		...
		{
			Require.That(templateId != null, new ErrorCode(@"SendgridError", @"The required Sengrid template ID not configured in app settings.", HttpStatusCode.NotImplemented));
			{
		...
		private async Task SendSupplierOrderSubmitEmails(HsOrderWorksheet orderWorksheet)
		{
			try
			{
				...
				foreach (var supplier in suppliers.Items)
				{
					...
					{
						// Get Order-Worksheet for supplier order and fill in some information from buyer order worksheet
		...
		public async Task EmailVoidAuthorizationFailedAsync(HsPayment payment, string transactionId, HsOrder order, CreditCardVoidException ex)
		{
			...
			catch (Exception ex1)
			{
				...
				LoggingNotifications.LogApiResponseMessages(_settings, SitecoreExtensions.Helpers.GetMethodName(), ResponseBody,
					LoggingNotifications.GetExceptionMessagePrefixKey(), true, ex1.Message, ex1.StackTrace, ex);
			}
		}
