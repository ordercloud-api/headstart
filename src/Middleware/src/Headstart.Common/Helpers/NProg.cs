...
namespace Headstart.Common.Helpers
{
	public class Tracker
	{
		...
		public void Every(Trigger trigger, Action<Progress> action)
		{
			_actions.Add(new ProgressAction { Trigger = trigger, Action = action, Recurring = true });
		}

		private void On(Trigger trigger, Action<Progress> action)
		{
			_actions.Add(new ProgressAction { Trigger = trigger, Action = action, Recurring = false });
		}

		public void Every(TimeSpan interval, Action<Progress> action)
		{
			_timers.Add(new TimerAction { Interval = interval, Action = action });
		}

		public void OnComplete(Action<Progress> action)
		{
			On(_total.ItemsDone(), action);
		}

		public void Now(Action<Progress> action)
		{
			action(GetProgress());
		}

		public void Every(Trigger trigger, Func<Progress, Task> action)
		{
			_actions.Add(new ProgressAction { Trigger = trigger, AsyncAction = action, Recurring = true });
		}

		private void On(Trigger trigger, Func<Progress, Task> action)
		{
			_actions.Add(new ProgressAction { Trigger = trigger, AsyncAction = action, Recurring = false });
		}

		public void Every(TimeSpan interval, Func<Progress, Task> action)
		{
			_timers.Add(new TimerAction { Interval = interval, AsyncAction = action });
		}

		public void OnComplete(Func<Progress, Task> action)
		{
			On(_total.ItemsDone(), action);
		}

		public void Now(Func<Progress, Task> action)
		{
			action(GetProgress());
		}
		...
		{
			ProcessTriggers(ref _started);
		}

		public void ItemSucceeded()
		{
			ProcessTriggers(ref _succeeded);
		}

		public void ItemFailed()
		{
			ProcessTriggers(ref _failed);
		}
		...
		{
			return new Progress(_startTime, _endTime, _total, _started, _succeeded, _failed);
		}
		...
		private void ProcessTriggers(ref int fieldToIncrement)
		{
			...
			lock (_lock)
			{
				...
				fired = _actions.Where(act => act.Trigger.IsFired(progress)).ToList();

				foreach (var act in fired.Where(act => !act.Recurring))
				{
					_actions.Remove(act);
				}
				_tasks.RemoveAll(t => t.IsCompleted);
			}
			fired.ForEach(act => act.Invoke(progress, _tasks));
		}
	...
	{
		public static Trigger ItemsStarted(this int i)
		{
			return new Trigger(i, p => p.ItemsStarted);
		}

		public static Trigger ItemsDone(this int i)
		{
			return new Trigger(i, p => p.ItemsDone);
		}

		public static Trigger ItemsSucceeded(this int i)
		{
			return new Trigger(i, p => p.ItemsSucceeded);
		}

		public static Trigger ItemsFailed(this int i)
		{
			return new Trigger(i, p => p.ItemsFailed);
		}

		public static Trigger PercentStarted(this int i)
		{
			return new Trigger(i, p => p.PercentStarted);
		}

		public static Trigger PercentDone(this int i)
		{
			return new Trigger(i, p => p.PercentDone);
		}

		public static Trigger PercentSucceeded(this int i)
		{
			return new Trigger(i, p => p.PercentSucceeded);
		}

		public static Trigger PercentFailed(this int i)
		{
			return new Trigger(i, p => p.PercentFailed);
		}
	...
	public class Progress
	{
		...
		private long SafeDivide(long x, long y)
		{
			return y == 0 ? 0 : x / y;
		}

		private double SafeDivideExact(long x, long y)
		{
