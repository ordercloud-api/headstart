...
namespace Headstart.Orchestration
{
    public class UserGroupAssignmentSyncCommand : SyncCommand, IWorkItemCommand
    {
        ...
        {
            var obj = wi.Current.ToObject<UserGroupAssignment>();
            try
            {
        ...
        {
            var obj = wi.Current.ToObject<UserGroupAssignment>(OrchestrationSerializer.Serializer);
            try
            {
                await _oc.UserGroups.SaveUserAssignmentAsync(wi.ResourceId, obj, wi.Token);
                return JObject.FromObject(obj);
        ...
        public async Task<JObject> GetAsync(WorkItem wi)
        {
            try
            {
                var response = await _oc.UserGroups.ListUserAssignmentsAsync(wi.ResourceId, userGroupID: wi.RecordId, wi.Token);
