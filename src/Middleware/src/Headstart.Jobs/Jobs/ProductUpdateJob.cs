...
{
    public interface IProductUpdateJob
    {
        Task Run(ILogger logger);
    }

    public class ProductUpdateJob : IProductUpdateJob
    {
        ...
        {
            if(!_settings.JobSettings.ShouldEmailProductUpdates)
            {
                logger.LogInformation("Skipped -- EmailProductUpdates");
                return;
            }
            logger.LogInformation("Starting function");
            try
            {
                await SendAllProductUpdateEmails();
                await CleanUpProductHistoryData();
            }
            catch (Exception ex)
            {
        ...
        private async Task SendAllProductUpdateEmails()
        {
        ...
        private ISheet SetHeaders(List<string> headers, ISheet worksheet)
        {
        ...
        private ISheet SetValues(IEnumerable<object> data, ISheet worksheet)
        {
            int i = 1;
        ...
        private async Task<List<ProductUpdateData>> BuildProductUpdateData()
        {
