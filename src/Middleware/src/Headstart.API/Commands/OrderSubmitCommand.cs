    ...
    public class OrderSubmitCommand : IOrderSubmitCommand
    {
        ...
        private async Task ValidateOrderAsync(HSOrderWorksheet worksheet, OrderCloudIntegrationsCreditCardPayment payment, string userToken)
        {
            try
            {
                ...
                var errors = ex.Errors.Where(ex => (!ex.ErrorCode.Equals($@"Order.CannotSubmitWithUnaccceptedPayments", StringComparison.OrdinalIgnoreCase)));
                if(errors.Any())
                {
                    ...
                    LogExt.LogException(_webConfigSettings.AppLogFileKey, Helpers.GetMethodName(), $@"{LoggingNotifications.GetGeneralLogMessagePrefixKey()}", $@"{ex.Message}. {ex1.Message}", ex1.StackTrace, this, true);
                }
            }
        }
