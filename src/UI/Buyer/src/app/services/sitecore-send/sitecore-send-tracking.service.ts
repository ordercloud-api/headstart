...
import { HSLineItem, HSMeProduct, HSProduct } from '@ordercloud/headstart-sdk'

// mootrack() is defined in a script loaded in loadMoosendTracker()
declare let mootrack: any
...
export class SitecoreSendTrackingService {
  ...
  identify(email: string): void {
    if (!this.appConfig.useSitecoreSend) {
      return
    }
    // this is not a real user
    if (email === this.anonymousUserEmail) {
      return
    }
  ...
  viewProduct(product: HSProduct | HSMeProduct): void {
    if (!this.appConfig.useSitecoreSend || !this.userIdentified) {
      return
    }

    const p = this.MapProduct(product)
  ...
  addToCart(lineItem: HSLineItem): void {
    ...
    const product = this.MapProductFromLi(lineItem)
    mootrack(
      'trackAddToOrder',
      product.itemCode,
      product.itemPrice,
  ...
  purchase(lineItems: HSLineItem[]): void {
    ...
    const products = lineItems.map(this.MapProductFromLi)
  ...
  private MapProduct(product: HSProduct | HSMeProduct): MoosendProduct {
    return {
      itemCode: product.ID,
