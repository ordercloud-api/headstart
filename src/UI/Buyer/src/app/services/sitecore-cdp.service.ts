...
export class SitecoreCDPService {
    constructor(
        ...
        if (!appConfig.useSitecoreCDP) { return; }
        this.loadCDPTracker();
    ...
    addToCart(lineItem: HSLineItem): void {
        ...
        event.product = {
            ...
            currency: this.userService.get().Locale.Currency,
    ...
    newPageViewed(): void {
        if (!this.appConfig.useSitecoreCDP) { return; }
        var event = this.buildBaseEvent("VIEW");
        this.sendEventToCDP(event);
    }
    ...
    clearCart(): void {
        if (!this.appConfig.useSitecoreCDP) { return; }
        var event = this.buildBaseEvent("CLEAR_CART");
        this.sendEventToCDP(event);
    }
    ...
    private buildBaseEvent(type: CDPEventType): BaseCDPEvent {
        ...
        return {
            ...
            type,
            language: currentUser.Locale.Language,
            currency: currentUser.Locale.Currency,
    ...
    private loadCDPTracker():void  {
        var clientKey = this.appConfig.sitecoreCDPApiClient;
        var target = this.appConfig.sitecoreCDPTargetEndpoint;
        var domain = this.appConfig.baseUrl;
